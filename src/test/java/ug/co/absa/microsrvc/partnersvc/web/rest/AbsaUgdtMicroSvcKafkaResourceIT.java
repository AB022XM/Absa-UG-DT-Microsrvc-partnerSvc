...
import ug.co.absa.microsrvc.partnersvc.IntegrationTest;
...
class AbsaUgdtMicroSvcKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/absa-ugdt-micro-svc-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/absa-ugdt-micro-svc-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/absa-ugdt-micro-svc-kafka/unregister"));
                return;
            }
        }
